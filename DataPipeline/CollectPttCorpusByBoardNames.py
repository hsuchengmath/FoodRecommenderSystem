...
import logging
logging.basicConfig(filename='CrawlPttError.log')
...
def ptt_crawl(Board_Name, start, page, column_eng):
    ...
    if column_eng is False:
        ...
        'article_id':ptt_aid,
            ...
            'date':ptt_date,
            'IP_address':ptt_ip,
            'total_comments_number':ptt_all,
            'boo':ptt_boo,
            'push':ptt_like,
            'neutral':ptt_neutral,
            'article_score(push-boo)':ptt_score,
            'total_comments':ptt_comment
        }
...
def CrawlPttData(Board_Name ='KoreaDrama' , start ='開始爬取的頁數' ,page_num= 5, column_eng=False):
...
def main():
    ...
    CrawlPttData(Board_Name =Board_Name , start =start ,page_num= page_num)
