...
class BertForSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, nsp_label=None, mlm_label=None):
        # bert layer
        ...
        # objective func 
        loss_NSP = self.loss_fct(pred_NSP.view(-1, self.NSP_num_labels), nsp_label.view(-1))
        loss_MLM = self.loss_fct(pred_MLM, mlm_label)
...
def BertIdToTensor(pair_sent_bert_id=list, segment_bert_id=list, mask_bert_id=list, vocab_num=int):
    bert_id_tensor = torch.tensor(pair_sent_bert_id)
    segments_tensor = torch.tensor(segment_bert_id, dtype=torch.long)
    try:
        onehot_mask_tensor = torch.sum(convert_id_to_onehot(torch.tensor(mask_bert_id), num_classes=vocab_num), axis=0)
    except:
        onehot_mask_tensor = None
...
def ConvertBertTokenizedBatch(batch_data=list, tokenizer='obj', config=dict, vocab_num=int):
    '''
    [[target_sent, next_sent, nsp_label, pair_sent_bert_id, segment_bert_id, mask_bert_id],...]
    '''
    bert_id_tensor_list, token_type_list, nsp_label_list, mlm_label_list = [], [], [], []
    for element in batch_data:
        # convert bert_id to tensor
        bert_id_tensor, token_type_tensor, onehot_mask_tensor = BertIdToTensor(pair_sent_bert_id=element[3], 
                                                                               segment_bert_id=element[4], 
                                                                               mask_bert_id=element[5],
                                                                               vocab_num=vocab_num)
        # check word number is less than max_charator_length
        if bert_id_tensor.shape[0] < config['max_charator_length'] and onehot_mask_tensor is not None:
    ...
    # concet batch sent and padding sent
    if len(bert_id_tensor_list) != 0:
        ...
        mlm_label_tensor = pad_sequence(mlm_label_list, batch_first=True).to(config['device'])
        ...
        nsp_label_tensor = torch.tensor([nsp_label_list]).to(config['device'])
        return bert_id_tensor, token_type_tensor, masks_tensors, nsp_label_tensor, mlm_label_tensor
    else:
...
train_data = DataProcess(config=config, tokenizer=tokenizer)
...
for epoch in range(config['epoch']):
    ...
    for i in tqdm(range(batfch_num)):
        ...
        bert_id_tensor, token_type_tensor, masks_tensors, nsp_label_tensor, mlm_label_tensor = \
            ConvertBertTokenizedBatch(batch_data=batch_data, config=config, vocab_num=vocab_num)
        ...
        if bert_id_tensor is not None:
            ...
            loss, loss_nsp, loss_mlm = bert_classifier(input_ids=bert_id_tensor, 
                                                       ...
                                                       nsp_label=nsp_label_tensor,
                                                       mlm_label=mlm_label_tensor)
            total_loss += loss
